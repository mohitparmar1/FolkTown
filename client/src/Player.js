...
export default class Player extends Phaser.GameObjects.Sprite {
  constructor(config) {
    ...
    // Player nickname text with better styling
    this.playerNickname = this.scene.add
      .text(this.x, this.y - 30, "YOU", {
        font: "14px monospace",
        fill: "#00ff00",
        stroke: "#000000",
        strokeThickness: 3,
    ...
    // Add glow effect to main player
    this.setTint(0xffffff); // Keep hero white (no tint)
  ...
  update(time, delta) {
    ...
    if (this.cursors.left.isDown) {
      ...
      // If we were moving in the previous frame but now stopped, this is a movement end -> check for encounters
      if (
        (prevVelocity.x !== 0 || prevVelocity.y !== 0) &&
        this.body.velocity.x === 0 &&
        this.body.velocity.y === 0
      ) {
        // Defensive: scene may not have attemptEncounter in other contexts
        if (this.scene && typeof this.scene.attemptEncounter === "function") {
          // Use the player's current world position
          this.scene.attemptEncounter(this.x, this.y);
        }
      }
    }
  }
      ...
      if (
        ...
        room.then((roomInstance) => {
          if (roomInstance && roomInstance.send) {
            roomInstance.send("PLAYER_CHANGED_MAP", {
              map: world.name,
            });
          }
        });
